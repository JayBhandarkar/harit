...
export async function GET(request) {
  ...
  if (PLACES_API_KEY) {
    try {
      const url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${userLat},${userLng}&radius=${radius}&type=park&key=${PLACES_API_KEY}`;
  ...
  return Response.json({ parks: fallback, source: 'fallback', note: 'Showing Delhi parks (API unavailable)' });
}
